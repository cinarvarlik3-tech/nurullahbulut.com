...
const items = [
  ...
  },
  {
    ...
    label: 'Instagram',
    href: INSTAGRAM_URL || '#',
    className: 'bg-[#E4405F] hover:bg-[#d62d4b] text-white'
  },
  {
    icon: <LinkedInLogo />,
    label: 'LinkedIn',
...
function FloatingButtonExample() {
  return (
    <FloatingButton
      ...
      {items.map((item) => (
        <FloatingButtonItem key={item.label}>
          <Link
            href={item.href}
            ...
            className={cn(
              'h-12 w-12 rounded-full flex items-center justify-center text-white shadow-md transition-colors',
              item.className
            )}
            title={item.label}
