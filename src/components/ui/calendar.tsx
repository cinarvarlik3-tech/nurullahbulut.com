...
function Calendar({
  ...
  type ChevronProps = { className?: string; size?: number; disabled?: boolean; orientation?: "up" | "down" | "left" | "right" };
  const defaultComponents = {
    Chevron: (props: ChevronProps) => {
      const { orientation, size = 16, className, ...rest } = props;
      if (orientation === "left") {
        return <ChevronLeft size={size} strokeWidth={2} className={className} aria-hidden="true" {...rest} />;
      }
      if (orientation === "up") {
        return <ChevronRight size={size} strokeWidth={2} className={className} aria-hidden="true" style={{ transform: "rotate(-90deg)" }} {...rest} />;
      }
      if (orientation === "down") {
        return <ChevronRight size={size} strokeWidth={2} className={className} aria-hidden="true" style={{ transform: "rotate(90deg)" }} {...rest} />;
      }
      return <ChevronRight size={size} strokeWidth={2} className={className} aria-hidden="true" {...rest} />;
    },
  };
